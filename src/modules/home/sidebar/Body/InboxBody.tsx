...
const SideBarInboxBody = ({ list, activeContactId }: IProps) => {
  const dispatch = useDispatch();

  const userId = useSelector((state: RootState) => state.user.user.id);

  const getContactInformation = (thread: IThread) => {
    if (thread.group) return thread.title;

    const { firstName, lastName, email } = getPrivateThreadRecipient(
      thread.users,
      userId
    );
...
export default SideBarInboxBody;
